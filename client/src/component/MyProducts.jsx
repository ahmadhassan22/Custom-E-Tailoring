...
export default function Dashproducts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`/api/product/getMyProducts/${myShop._id}`);
        if (!response.ok) {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        ...
        `/api/product/getProducts?shopId=${myShop._id}&startIndex=${startIndex}`
  ...
  return (
    <div className=" lg:flex flex-col justify-center items-center w-full table-auto overflow-x-scroll md:ax-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      <div className="flex gap-4 mx-auto items-center justify-between px-2">
        <Link to={`/shop/${myShop.slug}`}>
        <p className="italic   rounded-md  border-gray-500 text-gray-400 font-semibold ">{myShop.title}</p>
        </Link>
        <Link to={"/dashboard?tab=addproduct"}>
          <button className="bg-gradient-to-r bg-yellow-500
                    hover:scale-105 text-sm duration-200 text-white py-1 px-3 m-2 rounded-full" color={"gray"}>Add A Product</button>
        </Link>
        <Link to={`/shop/${myShop.slug}`}>
        
        <img className="w-14 rounded-full" src={myShop.image} alt="" />
        </Link>
      </div>
      {currentUser.isAdmin || currentUser.haveAShop ? (
        products.length > 0 ? (
          <>
              ...
              {products.map((shop, index) => (
                <Table.Body key={index} className="divide-y">
                  <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    {/* <Table.Cell>
                   <Link className="text-teal-500" to={`/update-shop/${shop._id}`}>
                        Edit
                      </Link>
                    </Table.Cell> */}
                  </Table.Row>
